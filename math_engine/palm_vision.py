...
# ─── FEATURE FLAGS ────────────────────────────────────────────────────────────
# Set True on localhost (8GB RAM) or FastAPI server.
...
def enhance_and_resize(image_array, target_width=900):
    """Resize to standard width and apply mild sharpening kernel."""
    h, w = image_array.shape[:2]
    resized = cv2.resize(
        image_array,
        (target_width, int(target_width * (h / w))),
...
def pad_to_square(crop_rgb, size=300, bg=(12, 8, 20)):
    """
    Pad any crop to a fixed dark square — fixes UI alignment.
    All mount tiles become identical size regardless of original dimensions.
    """
...
def remove_background(frame_rgb):
    """GrabCut background removal — skin-tone agnostic. Falls back to threshold."""
    try:
...
def apply_clahe_lab(frame_rgb):
    """
    CLAHE in LAB colour space — THE core accuracy fix.
    Makes palm lines pop before Frangi sees the image.
    Works on all skin tones (operates on Lightness channel only).
    Returns: (enhanced_rgb, enhanced_gray normalised 0-255).
    """
    lab = cv2.cvtColor(frame_rgb, cv2.COLOR_RGB2LAB)
    l, a, b = cv2.split(lab)
    l_enh = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8, 8)).apply(l)
...
def enhance_crop_for_ai(crop_rgb):
    ...
    Gemini needs to see lines — not flat skin tones.
    """
    if crop_rgb is None or (isinstance(crop_rgb, np.ndarray) and crop_rgb.size == 0):
        return crop_rgb
    lab = cv2.cvtColor(crop_rgb, cv2.COLOR_RGB2LAB)
    l, a, b = cv2.split(lab)
    l_enh = cv2.createCLAHE(clipLimit=5.0, tileGridSize=(4, 4)).apply(l)
    return cv2.cvtColor(cv2.merge([l_enh, a, b]), cv2.COLOR_LAB2RGB)


def get_skin_mask(frame_rgb):
    """
    HSV skin detection for all ethnicities.
    Applied to Frangi output to kill background noise.
    This fixed the 1127 fake endpoints from the previous version.
    """
...
# ══════════════════════════════════════════════════════════════════════
# SECTION 3 — ORIENTATION NORMALISATION
# ══════════════════════════════════════════════════════════════════════
...
def rotate_landmarks(lm, M, iw, ih):
    """Apply rotation matrix M to all landmark coordinate pairs."""
    out = {}
...
def run_dual_frangi(enhanced_gray):
    """
    Run Frangi for BOTH dark AND bright ridges, take pixel-wise max.
    Dark ridges  = normal palm lines on lighter skin (most common).
    Bright ridges = handles overexposed or unusual lighting.
    Prevents the all-lines-faint result from single-direction pass.
    Mild Gaussian denoising first so sensor noise is not amplified.
    """
    blurred = cv2.GaussianBlur(enhanced_gray, (3, 3), 0)
    g       = blurred.astype(np.float64) / 255.0
    r_dark  = frangi(g, sigmas=range(1, 9), black_ridges=True)
    r_brt   = frangi(g, sigmas=range(1, 9), black_ridges=False)
...
def build_persistence_map(frangi_norm):
    """
    8-threshold voting: each pixel scored 0-8 on how many thresholds it survives.
    Score 4+ = genuine line. Score 1-2 = faint crease or noise.
    Threshold lowered from 6 to 4 to handle moderate post-CLAHE signals.
    """
...
def isolate_lines_watershed(frangi_norm, persistence, skin_mask=None):
    ...
    Skin mask zeroes background before seeding.
    min_area raised from 1% to 5% of image height — removes micro-noise clusters.
    """
...
def extract_topology(clean_mask, img_h, img_w):
    """
    FIX 1: remove_small_objects(min_size=30) prunes micro-noise from skeleton
            before counting — this fixed the 1222 fake forks.
    FIX 2: Simian line requires a component to span BOTH the heart-line zone
            AND head-line zone — prevents false positives from a straight head line.
    """
    ...
    # Stricter Simian line: two-zone validation
    heart_y1  = int(img_h * 0.20)
    heart_y2  = int(img_h * 0.38)
    head_y2   = int(img_h * 0.55)
    simian    = False
    zone_skel = skel[heart_y1:head_y2, :]

    if zone_skel.sum() > 0:
        labeled = sk_label(zone_skel)
...
def extract_zone_scores(frangi_norm, lm, img_h, img_w):
    ...
    Coverage-weighted scoring formula:
      score = (fraction_covered x 0.6 + mean_intensity x 0.4) x 100
    Old formula gave 14 even when lines existed (35/255 x 100 = 13.7).
    New formula: a line covering 50% of zone at moderate intensity = ~40.
    ...
    def sc(zone):
        ...
        if len(px) == 0:
            return 0
        coverage  = len(px) / zone.size
        intensity = float(np.mean(px)) / 255.0
        return min(100, int((coverage * 0.6 + intensity * 0.4) * 100))
    ...
    hd1 = hy2
    hd2 = hd1 + int(ph * 0.22)
    lx1 = max(0,    tx - int(img_w * 0.13))
    lx2 = min(img_w, tx + int(img_w * 0.10))
    fx1 = max(0,    fx - int(img_w * 0.07))
    fx2 = min(img_w, fx + int(img_w * 0.07))
    sx1 = max(0,    sx - int(img_w * 0.06))
    sx2 = min(img_w, sx + int(img_w * 0.06))
...
def _fallback_zone_scores(frangi_norm, img_h, img_w):
    """Geometric thirds fallback when no landmarks detected."""
    def sc(zone):
        if zone.size == 0:
            return 0
        thr = max(12, int(np.percentile(zone, 40)))
...
def compute_finger_ratios(lm):
    """MediaPipe-based geometric finger measurements and hand type classification."""
    def fl(t, p):
        return float(np.linalg.norm(np.array(lm[t]) - np.array(lm[p])))

    il   = fl(8, 5)    # index finger length
    rl   = fl(16, 13)  # ring finger length
    ml   = fl(12, 9)   # middle finger length
    pl   = fl(20, 17)  # pinky length
    r244 = il / max(rl, 1)
...
def get_vitality_hsv(frame_rgb, lm, iw, ih):
    """HSV saturation/value sampling from Venus mount (base of thumb)."""
    vx, vy = lm[2]
    box    = max(40, int(iw * .07))
    crop   = frame_rgb[max(0, vy):min(ih, vy + box * 2),
                        max(0, vx - box):min(iw, vx + box)]
    if crop.size == 0:
        return "Balanced Kapha (Steady vitality)", "Balanced & Steady Energy"
    hsv = cv2.cvtColor(crop, cv2.COLOR_RGB2HSV)
    s   = float(cv2.mean(hsv)[1])
    v   = float(cv2.mean(hsv)[2])
    if s > 95 and v > 100:
        return "Active Pitta (High vitality — strong Venus mount)", "Warm, Passionate & High Energy"
    if s < 45 or v < 80:
        return "Cool Vata (Variable vitality)", "Cool, Sensitive & Variable Energy"
    return "Balanced Kapha (Steady vitality)", "Balanced & Steady Energy"
...
def _normalize_elevations(raw):
    """
    Normalize elevation scores relative to each other.
    Prevents all mounts showing Highly Developed when lighting is even.
    """
    if not raw:
        return {}
    mn = min(raw.values())
...
def estimate_mount_elevations_relative(orig_crops_raw):
    ...
    Fallback used on Streamlit Cloud where depth model is disabled.
    """
    raw = {}
...
def get_mount_elevations_depth(frame_rgb, lm):
    """
    Real monocular depth via Depth Anything V2 Small.
    Only runs when USE_DEPTH_MODEL = True (localhost or FastAPI server).
    """
    ...
    try:
        dep = np.array(
            estimator(PIL.Image.fromarray(frame_rgb))["depth"],
            np.float32
        )
...
def analyze_palm_hybrid(image_bytes):
    ...
    Master pipeline. Returns:
      palm_data       — full feature dict (zone scores, topology, geometry etc)
      diagnostic_xray — BGR numpy array (heatmap for UI display only)
    ...
    # CLAHE — core accuracy fix
    enhanced_rgb, enhanced_gray = apply_clahe_lab(best_frame)

    # Skin mask — kills fake endpoints from background noise
    skin_mask = get_skin_mask(best_frame)

    # Dual-ridge Frangi + apply skin mask
    ...
    # Depth elevation (localhost/server only)
    ...
    # Build diagnostic X-ray for UI display
...
def _draw_lm(bgr, lm):
    """Draw green landmark skeleton overlay on the diagnostic xray."""
    conns = [
        (0,1),(1,2),(2,3),(3,4),
        (0,5),(5,6),(6,7),(7,8),
...
def slice_xray_mounts(best_frame_rgb, diagnostic_xray, lm_dict, enhanced_rgb=None):
    """
    Slices all 7 planetary mounts. Returns three dicts (all square-padded):
      xray_crops     — labelled heatmap crops (UI display)
      orig_crops     — CLAHE-enhanced crops padded to square (UI what-AI-sees panel)
      orig_crops_raw — unpadded CLAHE crops (sent to Gemini Vision API)
    """
