"""features.numerology.view — Streamlit page (Full Report + Cycles & Pinnacles)."""
...
def show_numerology():
    ...
    with tab1:
        ...
        if mode == "Ask a Question":
            question = st.text_area("Your question", key="num_q",
                                    placeholder="e.g. When will my career take off?")
        ...
        if use_astro:
            ...
            with c2:
                ...
                num_dob = st.date_input("Date of Birth", pre_dob,
        ...
        if st.button("Generate Numerology Report ✨", type="primary", use_container_width=True):
            ...
            st.session_state.num_prompt = build_full_report_prompt(
                name, dob_str, lp, dest, soul, pers,
                dossier, question, system, knowledge_context=num_ctx,
            )
    ...
    with tab2:
        ...
        with c2:
            ...
            cyc_dob = st.date_input("Date of Birth", pre_dob2,
                                    min_value=date(1850, 1, 1), max_value=date(2050, 12, 31),
                                    key="cyc_dob")

        if st.button("Show My Cycles ✨", type="primary", use_container_width=True):
            if not cyc_name.strip():
                st.error("Enter your name."); return
            ...
            pd_ = get_personal_day(cyc_dob.isoformat(), user_tz)
            ...
            c3.metric("Personal Day", str(pd_))
            ...
            for i, (s, e, n, c) in enumerate([r1, r2, r3, r4], 1):
                ...
                col.markdown(f"**Pinnacle {i}** (Ages {s-y}–{e-y if e-y < 100 else '∞'}) &nbsp; {badge_html}",
                             unsafe_allow_html=True)
                col.write(f"**Pinnacle Number: {n}** — {PERSONAL_YEAR_MEANINGS.get(n, '')}")
                col.write(f"**Challenge Number: {c}** — {_CHALLENGE_MEANINGS.get(c, 'Master this cycle.')}")

            st.session_state.cyc_prompt = build_cycles_prompt(
                cyc_name.strip(), cyc_dob.isoformat(), lp, py, pm, pd_,
                [r1, r2, r3, r4], system=sys3,
            )
